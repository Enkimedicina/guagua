...
function App() {
  ...
  return (
    <div className="min-h-[100dvh] bg-gray-950 p-4 md:p-8 flex flex-col">
      <div className="max-w-7xl mx-auto w-full flex-1">
        <header className="mb-6 border-b border-gray-800 pb-4 flex items-center justify-between gap-3 sticky top-0 bg-gray-950/95 backdrop-blur z-20 pt-2">
          <div className="flex items-center gap-3">
            {/* Dynamic Header Icon based on Portfolio */}
            <div className={`${activePortfolioConfig.theme.primary} p-2 rounded-lg transition-colors duration-300 shadow-lg`}>
              <activePortfolioConfig.icon className="w-6 h-6 text-white" />
            </div>
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-white tracking-tight">USDT Pro</h1>
              <p className="text-gray-400 text-xs md:text-sm">Gestión de Inversiones</p>
            </div>
          </div>
        </header>

        {/* Portfolio Tabs - Scrollable on mobile */}
        <div className="mb-6 overflow-x-auto pb-2 -mx-4 px-4 md:mx-0 md:px-0 md:pb-0">
          <div className="flex space-x-2 bg-gray-900/50 p-1.5 rounded-xl w-full max-w-md border border-gray-800 min-w-max md:min-w-0">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 border-t border-gray-800 pt-6">
           <div>
             ...
             <p className="text-xs text-gray-500">
               Historial: <span className="text-gray-300">{activePortfolioConfig.label}</span>
             </p>
           </div>
           ...
           <div className="flex gap-3 w-full sm:w-auto">
             <button 
               ...
               className="flex-1 sm:flex-none bg-gray-800 hover:bg-gray-700 border border-gray-700 text-white px-4 py-3 rounded-xl flex items-center justify-center gap-2 shadow-lg transition-all active:scale-95 font-medium"
             >
               <Calculator className="w-5 h-5 text-gray-400" />
               <span className="md:inline">Simular</span>
             </button>

             {/* Dynamic Color Action Button */}
             <button 
               onClick={() => setIsTransactionModalOpen(true)}
               className={`flex-1 sm:flex-none ${activePortfolioConfig.theme.primary} ${activePortfolioConfig.theme.hover} text-white px-4 py-3 rounded-xl flex items-center justify-center gap-2 shadow-lg ${activePortfolioConfig.theme.shadow} transition-all active:scale-95 font-medium`}
             >
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center p-4 border-b border-gray-800 bg-gray-900 sticky top-0 z-10">
               <h3 className="text-white font-bold">Nueva Transacción</h3>
              <button 
            ...
            <div className="p-6 overflow-y-auto">
               ...
               <div className={`mb-6 ${activePortfolioConfig.theme.lightBg} border ${activePortfolioConfig.theme.border} p-3 rounded-lg flex items-center gap-2`}>
